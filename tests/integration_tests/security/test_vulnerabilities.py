...
def _build_booted_for_factory(factory, guest_kernel, rootfs, cpu_template, pci):
    """Build a VM via the given MicroVMFactory and boot it."""
    vm = factory.build(guest_kernel, rootfs, pci=pci)
    vm.spawn()
...
def _build_restored_for_factory(factory, guest_kernel, rootfs, cpu_template, pci):
    """Build a VM via the given MicroVMFactory, snapshot, and restore."""
    booted = _build_booted_for_factory(factory, guest_kernel, rootfs, cpu_template, pci)
    snapshot = booted.snapshot_full()
    booted.kill()
    restored = factory.build_from_snapshot(snapshot)
    restored.cpu_template_name = booted.cpu_template_name
    return restored


@pytest.fixture
def uvm_any_a(microvm_factory_a, uvm_lifecycle, guest_kernel, rootfs, cpu_template):
    ...
    """
    builder = (
        _build_booted_for_factory
        if uvm_lifecycle == "booted"
        else _build_restored_for_factory
    )
    return builder(microvm_factory_a, guest_kernel, rootfs, cpu_template, False)
