...
pub fn reset_to_snapshot(
    ...
    let reset_message = ResetSnapshotMessage {
            ...
            if pfn == prev_pfn + 1 {
                ...
                if madv_size > 4096 {
                    info!(
                        "Coalesced pages: {:x}-{:x}",
                        dirty_page_address,
                        dirty_page_address + madv_size as u64
                    );
                }
                // SAFETY: TBD
                let ret = unsafe {
                    libc::madvise(dirty_page_address as *mut _, madv_size, libc::MADV_DONTNEED)
                };
    ...
    Arc::get_mut(&mut vmm.vm)
        .expect("multiple references to VM")
